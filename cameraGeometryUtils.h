 ...
 */
CameraParameters getCameraParameters(AlgorithmParameters& algParams, CameraParameters_cu &cpc,
    ...
    if (!inputFiles.calib_filename.empty()) {
        ...
        readCalibFileKitti(inputFiles.calib_filename, params.cameras[0].P,
                           params.cameras[1].P);
        params.rectified =
            false;  // for Kitti data is actually rectified, set this to true
                    // for computation in disparity space
    }
    ...
    if (!inputFiles.krt_file.empty()) {
        ...
        readKRtFileMiddlebury(algParams, inputFiles.krt_file, params.cameras, inputFiles);
    }
    ...
    if (transformP) transform = getTransformationReferenceToOrigin(R[0], t[0]);
    ...
    for (size_t i = 0; i < numCameras; i++) {
        ...
        if (!inputFiles.bounding_folder.empty()) {
            Vec3f ptBL, ptTR;
            readBoundingVolume(inputFiles.bounding_folder +
                                   inputFiles.img_filenames[i] + ".bounding",
                               ptBL, ptTR);
        ...
        params.cameras[i].baseline = 1.0f;  // 0.54 = Kitti baseline
        ...
        copyOpencvMatToFloatArray(params.cameras[i].K_inv,
                                  &cpc.cameras[i].K_inv);
        copyOpencvMatToFloatArray(params.cameras[i].R_orig,
                                  &cpc.cameras[i].R_orig);
        copyOpencvMatToFloatArray(params.cameras[i].R_orig_inv,
        ...
        copyOpencvMatToFloatArray(params.cameras[i].P_inv,
                                  &cpc.cameras[i].P_inv);
        copyOpencvMatToFloatArray(params.cameras[i].M_inv,
                                  &cpc.cameras[i].M_inv);
