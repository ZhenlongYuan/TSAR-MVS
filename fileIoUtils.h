...
static void readKRtFileMiddlebury(AlgorithmParameters& algParams, const string filename, vector<Camera> cameras, InputFiles inputFiles)
{
    for (int i = 0; i < inputFiles.img_filenames.size(); i++) {
        string imgname = inputFiles.img_filenames[i];
        string numind = imgname.substr(0, 8);
        std::stringstream result_path;
        result_path << inputFiles.mslp_folder << "cams/" << numind << "_cam.txt";
        std::string result_folder = result_path.str();
        //std::cout << "result_folder: " << result_folder << endl;
        std::ifstream file(result_folder);
        std::string line;
        file >> line;
        Mat Rt;
        ...
        float depth_min;
        float depth_max;
        file >> depth_min >> interval >> depth_num >> depth_max;
        if (i == 0) {
            algParams.depthMin = depth_min;
            algParams.depthMax = depth_max;
        }
...
static void writeParametersToFile(char *resultsFile, InputFiles inputFiles,
                                  AlgorithmParameters &algParameters,
                                  GTcheckParameters &gtParameters,
                                  uint32_t numPixels, int Robthr, int Houthr,
                                  int minLineLength, int maxLineGap,
                                  int weaktextnum, float weakths) {
    ...
    myfile << "\nParameters:" << endl;
    myfile << "Robthr: " << Robthr << endl;
    myfile << "Houthr: " << Houthr << endl;
    myfile << "minLineLength: " << minLineLength << endl;
    myfile << "maxLineGap: " << maxLineGap << endl;
    myfile << "weaktextnum: " << weaktextnum << endl;
    myfile << "weakths: " << weakths << endl;
