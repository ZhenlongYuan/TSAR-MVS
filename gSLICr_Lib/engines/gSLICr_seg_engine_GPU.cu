...
__global__ void Cvt_Img_Space_device(const Vector4u* inimg, Vector4f* outimg,
                                     Vector2i img_size,
                                     COLOR_SPACE color_space);

__global__ void Enforce_Connectivity_device(const int* in_idx_img,
                                            int* out_idx_img,
                                            Vector2i img_size);
...
__global__ void Find_Center_Association_device(
    const Vector4f* inimg, const spixel_info* in_spixel_map, int* out_idx_img,
...
__global__ void Finalize_Reduction_Result_device(const spixel_info* accum_map,
                                                 spixel_info* spixel_list,
                                                 Vector2i map_size,
                                                 int no_blocks_per_spixel);

__global__ void Draw_Segmentation_Result_device(const int* idx_img,
                                                Vector4u* sourceimg,
                                                Vector4u* outimg,
                                                Vector2i img_size);
...
seg_engine_GPU::seg_engine_GPU(const settings& in_settings)
    ...
    if (in_settings.seg_method == GIVEN_NUM) {
        float cluster_size =
            (float)(in_settings.img_size.x * in_settings.img_size.y) /
...
gSLICr::engines::seg_engine_GPU::~seg_engine_GPU() { delete accum_map; }

void gSLICr::engines::seg_engine_GPU::Cvt_Img_Space(UChar4Image* inimg,
                                                    Float4Image* outimg,
                                                    COLOR_SPACE color_space) {
...
void gSLICr::engines::seg_engine_GPU::Find_Center_Association() {
    ...
    Find_Center_Association_device<<<gridSize, blockSize>>>(
        img_ptr, spixel_list, idx_ptr, map_size, img_size, spixel_size,
        gSLICr_settings.coh_weight, max_xy_dist, max_color_dist);
}
...
void gSLICr::engines::seg_engine_GPU::Update_Cluster_Center() {
    ...
    Update_Cluster_Center_device<<<gridSize, blockSize>>>(
        img_ptr, idx_ptr, accum_map_ptr, map_size, img_size, spixel_size,
        no_blocks_per_line);
...
void gSLICr::engines::seg_engine_GPU::Draw_Segmentation_Result(
    UChar4Image* out_img) {
    ...
    Draw_Segmentation_Result_device<<<gridSize, blockSize>>>(
        idx_img_ptr, inimg_ptr, outimg_ptr, img_size);
...
__global__ void Find_Center_Association_device(
    ...
    find_center_association_shared(inimg, in_spixel_map, out_idx_img, map_size,
                                   img_size, spixel_size, weight, x, y,
...
__global__ void Update_Cluster_Center_device(const Vector4f* inimg,
    ...
    if (x_offset < spixel_size * 3 && y_offset < spixel_size * 3) {
        ...
        if (x_img >= 0 && x_img < img_size.x && y_img >= 0 &&
            y_img < img_size.y) {
