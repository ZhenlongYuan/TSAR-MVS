...
{
	float dcolor = sqrtf((pix.x - center_info.color_info.x)*(pix.x - center_info.color_info.x)
	...
	float dxy = sqrtf((x - center_info.center.x) * (x - center_info.center.x)
			  + (y - center_info.center.y) * (y - center_info.center.y));


	float retval = dcolor * dcolor + (dxy * normalizer_xy * weight) * (dxy * normalizer_xy * weight);
	return sqrtf(retval);
}
